...
for date in guards:
    ...
    for x in range(60):
        if not guards[date]["sleep"]:
            guards[date]["shift"][i:] = [1 if last == "falls asleep" else 0 for x in range(i, 60 - i)]
            break
        if x == guards[date]["sleep"][0][0].minute:
            last = guards[date]["sleep"][0][1]
            guards[date]["shift"][i:x] = [0 if last == "falls asleep" else 1 for x in range(i, x)]
            i = x
...
id = max(sums.items(), key=lambda x: x[1])[0]
